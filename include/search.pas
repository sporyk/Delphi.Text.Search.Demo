...
var
 ...
 buf:PFileLocation;
begin
 while findNextFile(FQuery,location) do
  begin

   buf:=PFileLocation(AllocMem(SizeOf(TFileLocation)));
   buf^.path:=PWidechar(AllocMem(WStrLen(location.path)));
   WStrLCopy(buf^.path,location.path,WStrLen(location.path));
   buf^.data:=location.data;

   if Assigned(FOnNewItemFound) then FOnNewItemFound(TObject(Pointer(buf)));
  end;
